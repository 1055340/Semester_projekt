$(document).ready(function () {
    $('.challenger').each(function (i, obj) {
        if ($(this).data('accept') === 1) {
            console.log("this one is");
            var loc = $(this).children('.acc-btn');
            $(this).children('.acc-btn').empty();
            $('<a class="accept btn btn-success">&#128077;</a><a class="deny btn btn-danger">&#128078;</a>').hide().appendTo(loc).fadeIn(1000);
        } else if ($(this).data('won') === 1) {
            $(this).hide().appendTo('.won-challenges').fadeIn(1000);
        } else {
            return;
        }


    });

    $('.challenger').click(function () {
        $('#challengeModal .modal-body').empty();
        $('<img class="loader center-block animated swing infinite" src="/images/logo.png" /> <p style="text-align: center;">Loading...</p>').hide().appendTo('#challengeModal .modal-body').fadeIn(400);
        var id = $(this).data('id');
        ;
        $.ajax({
            url: '/Manage/GetChallengeDetails',
            type: 'Post',
            datatype: 'json',
            data: {
                order: 'Client_Call',
                id: id
            },
            success: function (data) {
                $('#challengeModal .modal-body').empty();
                var myJSON = JSON.stringify(data);
                var obj = eval('(' + myJSON + ')')[0];
                var ChallengerPercentageToWin = (obj.ChallengerValue / obj.ChallengeGoal) * 100;
                ChallengerPercentageToWin = Math.floor(ChallengerPercentageToWin);
                var ChallengedPercentageToWin = (obj.ChallengedValue / obj.ChallengeGoal) * 100;
                ChallengedPercentageToWin = Math.floor(ChallengedPercentageToWin);
                var date = new Date(parseInt(obj.ChallengeEnd.substr(6)));
                var challengeContent = "<div class='row'><div class='container-fluid' data-id=" + obj.ChallengerId + "><div class='challenge-name-container col-xs-12' ><span class='col-xs-10'>" + obj.ChallengerName + "</span></div><div class='col-xs-12 rival'><div class='progress-container col-xs-10'><div class='progress'><div class='progress-bar' role='progressbar' aria-valuenow=" + ChallengerPercentageToWin + " aria-valuemin='0' aria-valuemax='100' style='width:" + ChallengerPercentageToWin + "%;'>" + ChallengerPercentageToWin + "%</div></div></div><div class='col-xs-2'><img src='/images/challenge-goal.png'/></div></div></div><div class='container-fluid' data-id=" + obj.ChallengedId + "><div class='challenge-name-container col-xs-12'><span class='col-xs-10' >" + obj.ChallengedName + "</span></div><div class='col-xs-12 rival'><div class='progress-container col-xs-10'><div class='progress'><div class='progress-bar' role='progressbar' aria-valuenow=" + ChallengedPercentageToWin + " aria-valuemin='0' aria-valuemax='100' style='width:" + ChallengedPercentageToWin + "%;'>" + ChallengedPercentageToWin + "%</div></div></div><div class='col-xs-2'><img src='/images/challenge-goal.png'/></div></div></div></div><div class='row'><div class='col-xs-12'><h4><b>Title:</b></h4><h4>" + obj.ChallengeTitle + "</h4></div><div class='col-xs-12'><h4><b>Exercise:</b></h4><h4>" + obj.ExerciseName + "</h4></div></div><div class='row'><div class='col-xs-12'><h4><b>Goal:</b></h4><h4>" + obj.ChallengeGoal + " Kg / Km</h4></div></div><div class='row'><div class='col-xs-12'><h3 id='getting-started'></h3></div></div><div class='btn-container col-xs-12'><button class='cancel-exercise' type='button' data-dismiss='modal'>close</button></div>";

                $(challengeContent).hide().appendTo('#challengeModal .modal-body').fadeIn(400);

                //Hvis en af personerne har 0% progress, bliver baren lidt længere, så man kan se teksten i baren

                $('.progress-bar').each(function () {
                    if ($(this).text() === "0%") {
                        $(this).width(10 + "%");
                    }
                    var currentprogress = $(this).attr('aria-valuenow');
                    if (currentprogress >= 100 && obj.ChallengeEnded == false) {
                        var winner = 0;
                        $(this).attr('aria-valuenow', '100');
                        $(this).width('100%');
                        $(this).text('100%');
                        var winner = $(this).parent().parent().parent().parent().data('id');
                        $('<p>You Won!</p><btn class="reward-btn btn btn-success" data-toggle="modal" data-target="#challengeModal">Claim Reward</btn>').hide().prependTo('.btn-container').fadeIn(400);


                        $('.reward-btn').click(function () {
                            //Der er her blevet fundet en vinder af challengen, som nu laver et ajax kald for at kunne claime sit reward, og flytte challengen til afsluttede
                            $.ajax({
                                url: '/Manage/FinishChallenge',
                                type: 'Post',
                                datatype: 'json',
                                data: {
                                    order: 'Client_Call',
                                    winner: winner,
                                    id: obj.ChallengeId
                                },
                                success: function (data) {
                                    console.log("success")
                                    $("div[data-id='" + obj.ChallengeId + "']").hide().appendTo('.won-challenges').fadeIn(400);
                                    console.log($);
                                    console.log(data);
                                },
                                error: function (data) {
                                    console.log("error!");
                                    console.log(data);
                                },
                            });
                        });
                    } else if (obj.ChallengeEnded == true) {
                        $(this).parent().parent().parent().parent().parent().siblings().not($(this)).empty();
                        $('<div class="col-xs-12" style="text-align:center"><h3>This challenge has ended :)</h3></div>').hide().appendTo('.modal-body').fadeIn(400);
                    } else {
                        return;
                    }

                });

                $("#getting-started")
                    .countdown(date, function (event) {
                        $(this).text(
                            event.strftime('%D days %H:%M:%S')
                        );
                    })

            },
            error: function (data) {
                console.log("error!");
                console.log(data);
            },
        });

    });

    $('.accept').click(function () {
        var loc = $(this).parent();
        var id = $(this).parent().parent().data('id');
        $(this).parent().empty();
        $('<a class="" data-toggle="modal" data-target="#challengeModal">></a>').hide().appendTo(loc).fadeIn(1000);

        $.ajax({
            url: '/Manage/AcceptChallenge',
            type: 'Post',
            datatype: 'json',
            data: {
                order: 'Client_Call',
                id: id
            },
            success: function (data) {
                console.log("success")
                console.log(data);
            },
            error: function (data) {
                console.log("error!");
                console.log(data);
            },
        });
    });


    $('.deny').click(function () {
        console.log("wowo");
        var id = $(this).parent().parent().data('id');

        $(this).parent().parent().addClass('animated bounceOut');
        $(this).parent().parent().delay(500).fadeOut(300, function () {
            $(this).remove();
        });
        $.ajax({
            url: '/Manage/DeleteChallenge',
            type: 'Post',
            datatype: 'json',
            data: {
                order: 'Client_Call',
                id: id
            },
            success: function (data) {
                console.log("success")
                console.log(data);
            },
            error: function (data) {
                console.log("error!");
                console.log(data);
            },
        });
    });
    
});
//$('.challenger').click(function () {
//    console.log("wow");
//    //$(this).children('.acc-btn').children('.btn').click();
//    //$(this).children('.acc-btn').children('.btn').trigger('click');
//});